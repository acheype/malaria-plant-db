...
public class ExcelReader {
    ...
    public ReaderResult read(InputStream xlsDataInputStream) throws ImportException {
        ...
        try (InputStream inputXLS = new BufferedInputStream(xlsDataInputStream)) {
            ...
            for (SheetInfo sheetInfo : sheetInfos) {
                dtosMap.putList(sheetInfo.getDtoClass(),
                        (ArrayList<Object>) dtoBeans.get(sheetInfo.getDtoClass().getSimpleName()));
            }

            return new ReaderResult(dtosMap, cellErrors);

        } catch (IOException | InvalidFormatException e) {
            throw new ImportException("An unexpected errors occurs during the Excel file reading", e);
        } catch (RuntimeException e) {
            throw new ImportRuntimeException("An unexpected errors occurs during the Excel file reading", e);
        }
    }
    ...
    private XLSReader getJxlsReader() throws IOException, SAXException {
        ...
        // continue if an reading errors occurs
