...
angular.module('malariaplantdbApp')
    .directive('jhAlert', function(AlertService) {
		return {
            ...
            template: '<div class="alerts" ng-cloak="">' +
			                '<alert ng-cloak="" ng-repeat="alert in alerts" type="{{alert.type}}" close="alert.close()" data-ng-animate="\'fade\'">' +
                        '<pre>{{ alert.msg }}</pre></alert></div>',
			controller: ['$scope',
	            function($scope) {
    ...
    .directive('jhAlertError', function(AlertService, $rootScope) {
		return {
            restrict: 'E',
            template: '<div class="alerts" ng-cloak="">' +
			                '<alert ng-cloak="" ng-repeat="alert in alerts" type="{{alert.type}}" close="alert.close()" data-ng-animate="\'fade\'"><pre>{{ alert.msg }}</pre></alert>' +
			            '</div>',
			controller: ['$scope',
	            function($scope) {
	                $scope.alerts = AlertService.get();

					var cleanHttpErrorListener = $rootScope.$on('malariaplantdbApp.httpError', function (event, httpResponse) {
					    var i;
